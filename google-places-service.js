...
function bestCandidate(candidates, latitude, longitude) {
  ...
  candidates.forEach((candidate, popularityIndex) => {
    ...
    const score = typeScore + popularity + proximity;
    if (score > bestScore) {
...
export class GooglePlacesService {
  ...
  pendingGroups(locations) {
    if (!this.enabled) return [];
    if (Number(this.usage[monthKey()] || 0) >= MONTHLY_REQUEST_LIMIT) return [];
    const now = Date.now();
    return [...this.groupLocations(locations)].filter(([key]) => {
  ...
  hasPending(locations) {
    return this.pendingGroups(locations).length > 0;
  }
  ...
  async ensureClusters(locations, onProgress) {
    const pending = this.pendingGroups(locations);
    const month = monthKey();
    let completed = 0;
    for (const [key, center] of pending) {
      ...
      try {
        ...
        this.clusters.set(key, { fetchedAt: Date.now(), candidates });
      } catch (error) {
  ...
  find(latitude, longitude) {
    ...
    return bestCandidate(cached.candidates || [], latitude, longitude);
  }
